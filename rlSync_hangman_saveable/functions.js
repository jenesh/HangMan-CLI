...
// Makes new board array
const makeBoardArr = str => str.map(ele => ele);

// Updates the given letter between two arrays
const updateDynamicArr = (arr, ltr) =>
  arr.map(ele => (ele.toLowerCase() === ltr ? `${ele.toUpperCase()}` : ele));
// console.log(arr.map(ele => (ele === "ltr" ? true : false)));

// Updates static array with a $
const updateStaticArr = (arr, ltr) =>
  arr.map(ele => (ele === ltr ? "$" : `${ele}`));
...
const displayBoardArr = arr =>
  arr.map(ele => (ele === ele.toLowerCase() ? "_" : ele));
